"""
yc_preflight_check.py

Mirrors sr_preflight_check.py for the YC pipeline. Run this before turning
automatic mode on (or any time you're not sure things are still working):
  ...
  - The LinkedIn Chrome profile exists (does NOT confirm the login
    ...
    the profile directory itself is there)
  - Chrome/Chromium is actually installed and its version is detectable
  - The configured YC batch slug actually resolves (a real, free request
    to YC's public directory API; no credentials needed for this one)
...
def check_chrome_profile():
    ...
    print(
        ...
        "expire. If li_unified_leads.py's LinkedIn sources come back "
        "empty, re-run `python li_native_search.py --login-only`."
    )
...
def check_yc_batch_slug():
    config = load_config()
    slug = config["yc_batch_slug"]
    try:
        import yc_list_raw as yc
        companies = yc.fetch_batch_companies(slug)
        check(f"YC batch slug '{slug}' resolves", True, f"{len(companies)} companies currently public")
    except Exception as e:
        check(f"YC batch slug '{slug}' resolves", False, str(e))


def main():
    print("Running YC pipeline preflight check...\n")
    check_env_vars()
    check_smtp_login()
